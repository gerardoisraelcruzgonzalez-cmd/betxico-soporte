...
export async function claimLiveChatEvidenceResponse(chatId, evidenceId, record = {}) {
  const cleanChatId = String(chatId || "").trim();
  const cleanEvidenceId = String(evidenceId || "").trim();
  if (!cleanChatId || !cleanEvidenceId) {
    const error = new Error("missing_evidence_response_identifier");
    error.statusCode = 400;
    throw error;
  }
  const response = await kvRequest([
    "SET",
    evidenceResponseKey(cleanChatId, cleanEvidenceId),
    JSON.stringify({
      chatId: cleanChatId,
...
export async function saveLiveChatEvidenceResponse(chatId, evidenceId, record = {}) {
  const cleanChatId = String(chatId || "").trim();
  const cleanEvidenceId = String(evidenceId || "").trim();
  if (!cleanChatId || !cleanEvidenceId) return;
  await kvRequest(["SET", evidenceResponseKey(cleanChatId, cleanEvidenceId), JSON.stringify({
    chatId: cleanChatId,
    evidenceId: cleanEvidenceId,
    ...record,
    updatedAt: new Date().toISOString()
...
export async function releaseLiveChatEvidenceResponse(chatId, evidenceId) {
  const cleanChatId = String(chatId || "").trim();
  const cleanEvidenceId = String(evidenceId || "").trim();
  if (!cleanChatId || !cleanEvidenceId) return;
  await kvRequest(["DEL", evidenceResponseKey(cleanChatId, cleanEvidenceId)]);
}
...
export async function verifyLiveChatMessage({ chatId, eventId, text, visibility = "all" } = {}) {
  const cleanEventId = String(eventId || "").trim();
  const cleanText = normalizeMessageText(text);
  if (!cleanEventId || !cleanText) return false;
  const chat = await getLiveChat(chatId);
  return extractLiveChatTextMessages(chat).some((message) => (
    message.eventId === cleanEventId
    && message.authorType !== "customer"
    && normalizeMessageText(message.text) === cleanText
    && (!message.visibility || message.visibility === visibility)
  ));
}

export async function findLiveChatMessage({ chatId, text, visibility = "all", since } = {}) {
  const cleanText = normalizeMessageText(text);
  const sinceMs = parseTimestamp(since);
  if (!String(chatId || "").trim() || !cleanText || !sinceMs) return null;

  const chat = await getLiveChat(chatId);
  const lowerBoundMs = sinceMs - 60_000;
  const match = extractLiveChatTextMessages(chat).find((message) => {
  ...
  return match?.eventId ? { event_id: match.eventId } : null;
}
...
export function extractLiveChatTextMessages(chat = {}) {
  ...
  return dedupeMessages(events)
    .map(({ text, raw }) => ({
      ...
      createdAt: raw?.created_at || raw?.timestamp || "",
      eventId: raw?.id || raw?.event_id || "",
      visibility: String(raw?.visibility || raw?.event?.visibility || "").trim().toLowerCase()
    }))
    .filter((message) => message.text);
}
...
function evidenceResponseKey(chatId, evidenceId) {
  return `support:livechat:evidence-response:${chatId}:${evidenceId}`;
}
