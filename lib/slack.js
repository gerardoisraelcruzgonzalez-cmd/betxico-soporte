...
async function fetchSlackListItems({ token, listId, maxItems }) {
  ...
  while (items.length < maxItems && page < 8) {
    const body = {
      list_id: listId,
...
function readSlackItemDate(item) {
  const value = item?.updated_at || item?.updatedAt || item?.date_updated || item?.created_at || item?.createdAt || "";
  if (!value) return "";
