...
export async function createKycMutationJob({ ownerEmail, userId, operation, field = "", value = "", document = null }) {
  const cleanOwner = normalizeOwner(ownerEmail);
  const cleanUserId = String(userId || "").trim();
  const cleanOperation = String(operation || "").trim();
  if (!cleanOwner || !cleanUserId || !["edit", "upload"].includes(cleanOperation)) {
    throw Object.assign(new Error("invalid_kyc_mutation"), { statusCode: 400 });
  }
  const connectorMode = configuredConnectorAgent("kyc", ownerEmail) ? "agent" : "legacy";
  const id = crypto.randomUUID();
  const job = {
    id, ownerEmail: cleanOwner, authorizedOwners: [cleanOwner], userId: cleanUserId,
    operation: cleanOperation, field: String(field || "").trim(), value: String(value || "").trim(),
    document: document || null, connectorMode, status: "pending", createdAt: new Date().toISOString()
  };
  await command(["SET", `${PREFIX}job:${id}`, JSON.stringify(job), "EX", JOB_TTL_SECONDS]);
  await command(["RPUSH", queueKey(ownerEmail, connectorMode), id]);
  return job;
