...
function normalizeAddress(values = {}) {
  const nested = values.address && typeof values.address === "object" ? values.address : {};
  return {
    street: cleanKycText(values.street || values.calle || values.address_street || nested.street || nested.calle),
    exteriorNumber: cleanKycText(values.exterior_number || values.numero_exterior || values.address_number || nested.exterior_number || nested.numero_exterior),
    neighborhood: cleanKycText(values.neighborhood || values.colonia || values.address_neighborhood || nested.neighborhood || nested.colonia),
    postalCode: cleanKycText(values.postal_code || values.codigo_postal || values.address_postal_code || nested.postal_code || nested.codigo_postal),
    municipality: cleanKycText(values.municipality || values.municipio || values.address_municipality || nested.municipality || nested.municipio),
    state: cleanKycText(values.state || values.estado || values.address_state || nested.state || nested.estado)
  };
}
