...
async function closeSessions(job, reportProgress = async () => undefined, reportCheckpoint = async () => undefined) {
  ...
  await activePage.goto(BOB_SESSIONS_URL, { waitUntil: "domcontentloaded", timeout: 90_000 }).catch(async (error) => {
    const userField = activePage.locator("#SES00-read-user_identifier");
    if (!(await userField.count().catch(() => 0))) throw error;
  });
  if (await activePage.locator("input[type=password]").count()) throw new Error("bob_login_required");
  ...
  const sessions = await sessionsForCustomerInMonthlyBlocks(nativeClient, job.customerId, range);
...
async function verifyNoPendingSessions(nativeClient, customerId, range) {
  ...
  for (let attempt = 0; attempt < 5; attempt += 1) {
    sessions = await sessionsForCustomerInMonthlyBlocks(nativeClient, customerId, range);
...
async function sessionsForCustomerInMonthlyBlocks(nativeClient, customerId, range) {
  const sessions = [];
  for (const block of monthlyBlocks(range)) {
    sessions.push(...await nativeClient.sessionsForCustomer(customerId, block));
    await wait(150);
  }
  return uniqueByNativeSession(sessions);
}

function monthlyBlocks({ startDate, endDate }) {
  const blocks = [];
  let cursor = new Date(`${startDate}T00:00:00Z`);
  const end = new Date(`${endDate}T00:00:00Z`);
  while (cursor <= end) {
    const blockStart = cursor.toISOString().slice(0, 10);
    const blockEndDate = new Date(Date.UTC(cursor.getUTCFullYear(), cursor.getUTCMonth() + 1, 0));
    if (blockEndDate > end) blockEndDate.setTime(end.getTime());
    blocks.push({ startDate: blockStart, endDate: blockEndDate.toISOString().slice(0, 10) });
    cursor = new Date(Date.UTC(cursor.getUTCFullYear(), cursor.getUTCMonth() + 1, 1));
  }
  return blocks;
}

function uniqueByNativeSession(sessions) {
  const seen = new Set();
